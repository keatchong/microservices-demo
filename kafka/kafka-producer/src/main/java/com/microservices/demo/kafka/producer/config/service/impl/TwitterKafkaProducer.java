   ...
   @PreDestroy
    ...
    private void addCallback(String topicName, TwitterAvroModel message,
    	...
    	kafkaResultFuture.whenComplete(
    			new BiConsumer<SendResult<Long,TwitterAvroModel>,Throwable>()
    			{

    		        @Override
    		        public void accept(SendResult<Long, TwitterAvroModel> result, Throwable u) {
    		            if (u != null) {
    		            	LOG.error("Error while sending message {} to topic {}", message.toString(), topicName, u);
    		            }
    		            else {
